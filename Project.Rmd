...
As we expected the classification model achieved a very high accuracy rate of `r round(result$overall[1] * 100, 2)`%, so our estimate of the out of sample error is `r round((1 - result$overall[1]) * 100, 2)`% with `r round((1 - result$overall[4: 3]) * 100, 2)`% confidence interval. 
